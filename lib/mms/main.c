...
void openMedia() {
    ...
    printf("file name:: %s, extension %s, n stream: %d\n",mediaContext->url,mediaContext->iformat->name,mediaContext->nb_streams);
    ...
    transcode(mediaContext,OUT_FORMAT,OUT_VIDEO_PATH);
...
void transcode(AVFormatContext *input_fmt_ctx, char * out_fmt, char *out_path){
    ...
    while(1){
        ...
        av_packet_rescale_ts(av_packet,input_fmt_ctx->streams[av_packet->stream_index]->time_base,out_ctx->streams[av_packet->stream_index]->time_base);
         av_packet->pos = -1;
